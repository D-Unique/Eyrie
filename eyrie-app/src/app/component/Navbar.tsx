...
const Navbar: React.FC = () => {
  ...
  return (
    <header className={`flex fixed top-0 left-0 right-0 z-50 w-full bg-black shadow-md  opacity-90 ${open ? 'max-sm:h-44 max-sm:opacity-100' : ''} `}>
        ...
        <div className={`hidden  max-sm:items-center max-sm:justify-start  max-sm:flex-col  max-sm:h-[1000px]  max-sm:mt-4 ${open ? 'max-sm:flex' : 'hidden'} `}>
        ...
        <div>
        <FontAwesomeIcon icon={faBars} onClick={() => setOpen(true)} className={` text-[#e6bf33] text-2xl  ml-40 mt-3 cursor-pointer ${open ? 'hidden' : ''}`} />
          
        <FontAwesomeIcon icon={faClose} className={`text-[#e6bf33] text-2xl  mr-8 mt-8 cursor-pointer ${open ? '' : 'hidden'}` } onClick={() => setOpen(false)}/>
        </div>
        </div>
